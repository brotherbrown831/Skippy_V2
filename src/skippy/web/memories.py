...
async function loadMemories() {
  ...
  params.set('order', sort === 'created_at' || sort === 'updated_at' ? 'desc' : 'desc');

  try {
    const res = await fetch('/api/memories?' + params);
    const data = await res.json();
    countEl.textContent = data.length + ' memor' + (data.length === 1 ? 'y' : 'ies');
    tbody.innerHTML = data.map(m => `
      <tr>
        ...
        <td><button class="btn-del" onclick="del_(${m.memory_id})">Delete</button></td>
      </tr>
    `).join('');
  } catch (err) {
    tbody.innerHTML = '<tr><td colspan="6">Failed to load memories.</td></tr>';
  }
}
